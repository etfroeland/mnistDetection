...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray):
    ...
    C_n = np.zeros((N, 1))
    for n in range(N):
        C_n[n] = -np.sum(targets[n]*np.log(outputs[n]))
    C = 1/(N)*np.sum(C_n)
    return C
...
class SoftmaxModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        z = np.dot(X, self.w)
        y = (np.exp(z).T / np.sum(np.exp(z),axis=1)).T
        return y
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        self.grad = -(np.dot(X.T, targets-outputs).T / (targets.shape[0])).T
        self.grad += 2*np.dot(self.l2_reg_lambda, self.w)
...
def one_hot_encode(Y: np.ndarray, num_classes: int):
    ...
    """
    targets = np.zeros((Y.shape[0], num_classes), int)
